...
# Prevent HuggingFace tokenizers from deadlocking in Streamlit
...
def update_similarity_in_db(resume_name):
    ...
    try:
        ...
        progress_text.text(f"Calculating similarity for {total_jobs} jobs against '{resume_name}'...")

        # 2. Calculate similarities sequentially to prevent PyTorch thread collisions
        similarity_results = {}
        skipped_count = 0
        zero_emb_count = 0
        start_time = time.time()

        for index, job_tuple in enumerate(jobs_data):
            job_id, score = _calculate_similarity_for_job(job_tuple, resume_embedding_np)
            if score is None:
                skipped_count += 1
            elif score == 0.0:
                zero_emb_count += 1
            ...
            if (index + 1) % 50 == 0 or (index + 1) == total_jobs:
